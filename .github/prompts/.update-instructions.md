## Prompt: Maven Project – Upgrade Vulnerable Libraries
...
Once completed, analyze the generated report (typically found in target/dependency-check-report.html) and perform the following actions:
    ...
    - Exclude provided scope (the dependencies in pom.xml that have <scope>provided</scope>) and test scope (<scope>test</scope>) dependencies from the list, as they are not packaged with the final artifact.

    - Exclude CVEs that are older than 1 year from the current date, as they are likely false positives or already addressed upstream.
